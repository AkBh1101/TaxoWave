...
// ---------- Data model (mirrors backend/pipeline.py build_report) ----------
interface Overview {
  run_seed: number;
  ...
  novel_pct: number;
  known_taxa_count: number;
  novel_cluster_count: number;
  total_taxonomic_units: number;
}
interface TaxonomyComposition { phylum: string; reads: number; }
interface GroupComposition { group: string; count: number; }
interface Station {
  station_id: string; station_name: string; latitude: number; longitude: number;
  depth_m: number; sequence_count: number; richness: number; shannon: number;
  simpson: number; evenness: number; chao1: number; novel_taxa: number;
}
interface NovelCluster {
  cluster: string; sequence_count: number; total_reads: number; stations: string[];
  station_count: number; max_reference_similarity: number; novelty_score: number;
  representative_sequence: string; points: { x: number; y: number }[];
}
interface TopTaxon { taxon: string; reads: number; }
interface Report {
  overview: Overview;
  taxonomy_composition: TaxonomyComposition[];
  group_composition: GroupComposition[];
  stations: Station[];
  novel_clusters: NovelCluster[];
  top_taxa: TopTaxon[];
}
interface RunResponse { duration_ms: number; report: Report; }
...
// =====================================================================
function initAmbientField(): void {
...
// Count-up
// =====================================================================
function animateCount(el: Element, target: number, duration = 1200): void {
...
// =====================================================================
let phylumChart: any = null;
let groupChart: any = null;
let topTaxaChart: any = null;
let map: any = null;
...
function renderHeroCounters(o: Overview): void {
...
function renderStatGrid(o: Overview): void {
  ...
  grid.innerHTML = stats.map(([num, lbl]) =>
    `<div class="stat-tile"><div class="num">${num}</div><div class="lbl">${lbl}</div></div>`
  ).join("");
}

function renderPhylumChart(data: TaxonomyComposition[]): void {
  if (phylumChart) phylumChart.destroy();
  phylumChart = new Chart(document.getElementById("phylumChart"), {
    ...
    options: {
      ...
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c: any) => c.parsed.x.toLocaleString() + " reads" } } },
      scales: { x: { grid: { color: GRID }, ticks: { callback: (v: any) => Number(v).toLocaleString() } }, y: { grid: { display: false } } },
    },
  });
}

function renderGroupChart(data: GroupComposition[]): void {
  ...
  if (!groupChartRafStarted) {
    ...
    (function spin() {
      ...
      if (groupChart) { groupChart.options.rotation = groupChartRotation; groupChart.update("none"); }
      requestAnimationFrame(spin);
    })();
  }
}

function renderTopTaxaChart(data: TopTaxon[]): void {
  if (topTaxaChart) topTaxaChart.destroy();
  topTaxaChart = new Chart(document.getElementById("topTaxaChart"), {
    ...
    options: {
      ...
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: (c: any) => c.parsed.x.toLocaleString() + " reads" } } },
      scales: {
        x: { grid: { color: GRID }, ticks: { callback: (v: any) => Number(v).toLocaleString() } },
...
function renderStationList(stations: Station[]): void {
...
function renderMap(stations: Station[]): void {
...
function renderClusterCards(clusters: NovelCluster[]): void {
...
// =====================================================================
// Orbit visualization — each novel cluster revolves around the
// "unclassified pool" core; radius = novelty score, size = sequence count.
// =====================================================================
interface OrbitBody { name: string; color: string; radius: number; angle: number; speed: number; size: number; count: number; }
let orbitBodies: OrbitBody[] = [];
let orbitRafStarted = false;

function setOrbitData(clusters: NovelCluster[]): void {
...
function initClusterOrbit(): void {
  const canvas = document.getElementById("clusterOrbit") as HTMLCanvasElement | null;
  if (!canvas) return;
  const ctx = canvas.getContext("2d")!;
  ...
  if (!orbitRafStarted) { orbitRafStarted = true; draw(); }
}
...
function consoleLine(text: string, cls = ""): HTMLDivElement {
...
function setStatus(state: "idle" | "running" | "done"): void {
  const badge = document.getElementById("pipeline-status")!;
  const dot = badge.querySelector(".status-dot") as HTMLElement;
  const label = badge.querySelector(".status-label") as HTMLElement;
  if (state === "idle") { label.textContent = "Idle"; dot.style.background = "#72908D"; }
  if (state === "running") { label.textContent = "Running"; dot.style.background = "#F2C078"; }
  if (state === "done") { label.textContent = "Complete"; dot.style.background = "#4FE7C1"; }
}

async function runPipelineWithConsole(): Promise<void> {
  ...
  const fetchPromise = postJSON<RunResponse>("/api/run-pipeline");
  ...
  logEl.appendChild(consoleLine(
    `✓ pipeline complete in ${result.duration_ms}ms — ${o.total_sequences} sequences · ${o.classified_pct}% matched · ${o.novel_cluster_count} novel clusters flagged (seed ${o.run_seed})`,
    "console-done"
  ));
  logEl.scrollTop = logEl.scrollHeight;
...
function initCustomCursor(): void {
  ...
  document.addEventListener("mouseover", (e) => {
    ...
    if (t.closest(hoverTargets)) ring.classList.add("is-active");
  });
  document.addEventListener("mouseout", (e) => {
    const t = e.target as HTMLElement;
    if (t.closest(hoverTargets)) ring.classList.remove("is-active");
  });
}
...
(async function main() {
  ...
  if (btn) btn.addEventListener("click", () => { runPipelineWithConsole(); });

  let report: Report;
  try {
    report = await getJSON<Report>("/api/report");
  } catch (e) {
    const sub = document.querySelector(".hero-sub");
    if (sub) sub.textContent = "Could not reach the TaxoWave API. Start the backend (uvicorn main:app --reload) and reload this page.";
    return;
  }
  renderReport(report);
